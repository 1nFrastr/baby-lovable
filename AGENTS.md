...
Chat + live preview iframe. Same sessions and workspaces as CLI. Type `/` in the composer for slash commands (`/summarize` seals older turns into a persisted summary without starting a builder turn). Use the UI when you want a human visual pass; **do not require it** for agent verification.
