...
- \`checkPreview\` only probes HTTP readiness — it does not start the preview and does not read compile logs (the platform warms preview in the background; compile diagnosis is on write/edit). **Before finishing any turn that edited files, you MUST call \`checkPreview\` and wait until \`ok: true\` / \`status: ready\` at least once** (first turn almost always needs this — do not claim the app is done while preview is still \`installing\` / \`starting\`). After that successful ready check in the session, small follow-up edits can rely on HMR and skip end-of-turn \`checkPreview\`, unless you ran \`installPackage\` / \`installDependencies\`, changed root config, did a large structural rewrite, got a \`compileError\`, or the user asks to verify.
