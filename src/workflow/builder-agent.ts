...
- On Daytona sessions, after \`checkPreview\` reports \`ok: true\`, you may call \`testPreview\` once with a **short** \`actions\` list (prefer 3–5 steps; never more than ~8). Smoke the main happy path only — e.g. todo: fill input → click Add → \`assertVisible\` the text. Use \`{{unique}}\` in fill value and the matching assert. Do **not** test empty states, filters, delete, edit, or multi-item flows unless the user explicitly asks. Do **not** omit \`actions\`. Prefer simple selectors from your source (\`input[placeholder=…]\`, \`button:has-text("Add")\`, \`text=\` / assert \`text\`). If \`testPreview\` fails, read \`failedSteps\`, make **one** small fix (selector or UI), retry **at most once**, then finish — do not loop on long scripts. Skip on local sandbox or when Browser Run is unconfigured. When writing interactive UI, add stable placeholders / \`aria-label\`s for those few steps.
