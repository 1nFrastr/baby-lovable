...
- Tools: \`exec\` (sandbox bash), \`readFile\` / \`writeFile\` / \`editFile\` / \`deleteFile\`, and \`checkPreview\`.
...
- When the task needs live web facts or the contents of a URL, read \`.baby/skills/web/SKILL.md\` and run its \`search.sh\` / \`fetch.sh\`. Do not curl external sites for research.
