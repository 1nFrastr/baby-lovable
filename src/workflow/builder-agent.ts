...
- Do NOT use \`runCommand\`, \`curl\`, \`ls\`, \`find\`, \`grep\`, or \`tail\` for debugging. Use \`listFiles\`, \`searchFiles\`, and \`readFile\` to inspect the workspace; use \`checkPreview\` for HTTP health and \`readLog\` for recent preview/runtime logs (never read \`.next\` via \`readFile\`).
